...
abstract contract ZunamiElasticRigidVault is AccessControl, ElasticRigidVault, RigidAddressSet {
    ...
    function redistribute() public {
        uint256 totalRigidNominal = _convertToNominalWithCaching(totalSupplyRigid(), Math.Rounding.Up);

        require(lockedNominalRigid() >= totalRigidNominal, "Wrong redistribution total nominal balance");

        uint256 nominal = lockedNominalRigid() - totalRigidNominal;

        SafeERC20.safeApprove(IERC20Metadata(asset()), address(redistributor), nominal);
